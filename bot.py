...
"""
════════════════════════════════════════════════════════════
        OTP PANEL BOT — SUPREME MASTER EDITION v4.2
   Full Admin Commands + Render 24/7 + Premium Keyboard
════════════════════════════════════════════════════════════
"""
...
def get_reply_menu(is_admin: bool, bot_token: str, chat_id: int = 0) -> ReplyKeyboardMarkup:
    """Restored the Main Keyboard exactly as you requested!"""
    keys = [
    ...
    if text == "🔑 Recent OTPs":
        ...
        for d in devices:
            ...
            for sms in smss:
                ...
                otp = extract_otp(body)
                if otp:
                    found.append((otp, device_label(d), sms.get("sender") or "?", sms_date(sms), d.db_tag))
...
def main() -> None:
    keep_alive()  # <--- RENDER SERVER (Don't remove)
    
    if not TOKEN:
        raise SystemExit("❌ TOKEN missing!")

    print("═" * 56)
    print("  🤖 OTP PANEL BOT — SUPREME MASTER EDITION v4.2")
    print("  🚀 Render Server + Admin Panel + VIP Features")
    print("═" * 56)
