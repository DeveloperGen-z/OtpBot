...
REQUIRED_CHANNELS = [
    {"username": "earnflowspidy", "url": "https://t.me/earnflowspidy", "name": "Earn-Flow"},
    {"username": "Ayuxh_spotted01", "https://t.me/Ayuxh_spotted01": "https://t.me/Ayuxh_spotted01", "name": "Ayuxh-Spotted"},
]
...
# ════════════════════════════════════════════════════════════
#  CHANNEL CHECK (FORCE JOIN LOGIC)
# ════════════════════════════════════════════════════════════

async def check_membership(bot_token, bot, user_id: int) -> list[str]:
    """Checks if the user is in all REQUIRED_CHANNELS."""
    if bot_token != TOKEN:
        return []
    not_joined = []
    for ch in REQUIRED_CHANNELS:
        try:
            ...
            # If API fails (e.g. bot not admin), assume not joined to enforce rule
            not_joined.append(ch["username"])
    return not_joined


async def send_join_prompt(update: Update, is_main_bot: bool) -> None:
    """Sends the Inline Keyboard with Join buttons."""
...
async def on_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # STRICT FORCE JOIN CHECK FOR ADMIN COMMANDS
    if is_main_bot:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
...
async def _dispatch_callback(query, ctx, data: str, chat_id: int, bot_token: str,
    ...
    # STRICT FORCE JOIN CHECK FOR ALL INLINE BUTTONS (Except Check Join & Close)
    if data != "check_join" and is_main_bot:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
            await safe_answer(query, "🚨 Aapne channel leave kar diya hai! Pehle channels join karein.", show_alert=True)
            try:
                if query.message:
                    await query.message.delete()
            except Exception:
                pass
            await send_join_prompt(ctx, is_main_bot) # Passing update context equivalent
            return
        users_db.setdefault(chat_id, {})["verified"] = True
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # STRICT FORCE JOIN CHECK FOR TEXT MESSAGES
    if is_main_bot:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
